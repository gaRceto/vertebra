...
if(isset($json->email)){
	$name=ucwords(strtolower(trim($json->name)));
	$apellidos=ucwords(strtolower(trim($json->apellidos)));
	$dni=strtoupper(trim($json->dni));
	$profesion=ucwords(strtolower(trim($json->profesion)));
	...
	try{
				...
				if($resultado['dni']==$dni)
					die("Ya has firmado el manifiesto");
				...
				$headers = "From: info@vetebraragon.com\r\n";
				...
				$mensaje .="<a href='https://vertebraragon.com/sis/bofir.php?id=".$elid."'>Borrar firmante</a>";
				mail("info@vertebraragon.com",$asunto,$mensaje,$headers);
				...
				$line.="<div>Provincia</div>".PHP_EOL;
				$line.="</div>".PHP_EOL;
